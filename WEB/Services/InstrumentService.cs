...
public class InstrumentService : IInstrumentService
{
            ...
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    InstrumentViewModel inst = new InstrumentViewModel
                    {
                        ...
                        ToolInventory = dr["ToolInventory"].Equals(DBNull.Value) ? null : dr["ToolInventory"].ToString(),
                        ReqDueDate = dr["ReqDueDate"].Equals(DBNull.Value) ? null : Convert.ToDateTime(dr["ReqDueDate"]),
                        InspectionCount = Convert.ToInt32(dr["InspectionCount"]),
                        RequestCount = Convert.ToInt32(dr["RequestCount"]),
	...
	public DataSet GetInstrumentsId(int InstrumentId)
	{
		...
		SqlCommand command = new SqlCommand("GetInstrumentForId");
    ...
    public async Task<bool> MailInsertInstrument()
    {
        try
        {
            InstrumentViewModel instrumentdata = JsonConvert.DeserializeObject<InstrumentViewModel>(_contextAccessor.HttpContext.Session.GetString("Instrument"));
            //RequestViewModel Request = JsonConvert.DeserializeObject<RequestViewModel>(_contextAccessor.HttpContext.Session.GetString("Request"));
        ...
        string mailSubject = "New Instrument Calibration Request /  新規計量器校正依頼の件 - " + ReqestNo + "";
        ...
        if (Deptdata != null)
        {
			...
			if (objToolInventory != "Yes")
            {
                ...
                foreach (var item in fmUserById)
                {
                    ...
                    mailbodyLab = mailbodyLab.Replace("$NAME$", item.FirstName + " " + item.LastName).Replace("$USERNAME$", labUserById.FirstName + " " + labUserById.LastName).Replace("$REQNO$",ReqestNo).Replace("$TYPEREQUEST$", RequestType).Replace("$INSTRUMENTNAME$", instrumentdata.InstrumentName).Replace("$INSTRUMENTID$", instrumentdata.IdNo).Replace("$REQNAME$", labUserById.FirstName + " " + labUserById.LastName).Replace("$DATE$", Convert.ToString(instrumentdata.CreatedOn)).Replace("$REQNAME$", labUserById.FirstName + " " + labUserById.LastName).Replace("$REQDEPT$", labUserById.DepartmentName).Replace("$SUBSECTION$", SubSectionEng).Replace("$SUBSECTIONJP$", SubSectionJP);
                ...
                if (LabDeptUser == 0)
                {
                    ...
                    mailbodyUser = mailbodyUser.Replace("$NAME$", labUserById.FirstName + " " + labUserById.LastName).Replace("$USERNAME$", labUserById.FirstName + " " + labUserById.LastName).Replace("$REQNO$", ReqestNo).Replace("$TYPEREQUEST$", RequestType).Replace("$INSTRUMENTNAME$", instrumentdata.InstrumentName).Replace("$INSTRUMENTID$", instrumentdata.IdNo).Replace("$REQNAME$", labUserById.FirstName + " " + labUserById.LastName).Replace("$DATE$", Convert.ToString(instrumentdata.CreatedOn)).Replace("$REQNAME$", labUserById.FirstName + " " + labUserById.LastName).Replace("$REQDEPT$", labUserById.DepartmentName).Replace("$SUBSECTION$", SubSectionEng).Replace("$SUBSECTIONJP$", SubSectionJP);
            ...
            _contextAccessor.HttpContext.Session.SetString("Instrument", string.Empty);
    ...
    public ResponseViewModel<InstrumentViewModel> InsertInstrument(InstrumentViewModel instrument)
    {
        try
        {
            ...
            _contextAccessor.HttpContext.Session.SetString("Instrument", JsonConvert.SerializeObject(instrument));
                
            _contextAccessor.HttpContext.Session.SetString("TypeOfRequest", newRequest.TypeOfReqest.ToString());
            _contextAccessor.HttpContext.Session.SetString("RequestNo", newRequest.ReqestNo);

            _contextAccessor.HttpContext.Session.SetString("InstrumentName", instrument.InstrumentName);
            _contextAccessor.HttpContext.Session.SetString("CreatedOn", instrument.CreatedOn.ToString());

            if (instrument.IdNo == null)
            {
                instrument.IdNo = string.Empty;
            }

            _contextAccessor.HttpContext.Session.SetString("IdNo", instrument.IdNo);
            _contextAccessor.HttpContext.Session.SetString("UserDept", instrument.UserDept.ToString());
    ...
    public ResponseViewModel<InstrumentViewModel> UpdateInstrument(InstrumentViewModel instrument)
    {
        ...
        try
        {
            ...
            if (instrument.AmountJPY != null)
            {
                instrumentById.AmountJPY = instrument.AmountJPY;
            }
            ...
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    InstrumentViewModel inst = new InstrumentViewModel
                    {
                        ...
                        UserRoleId = userRoleId,
                        TypeOfEquipment = dr["TypeOfEquipment"].ToString(),
                        QuaraantineOn = dr["QuarantineCreatedOn"].Equals(DBNull.Value) ? null : Convert.ToDateTime(dr["QuarantineCreatedOn"]),
  ...
  public ResponseViewModel<InstrumentViewModel> InstrumentRemoveQuarantine(int instrumentId, int statusId, int userId)
  {
	...
	public ResponseViewModel<InstrumentViewModel> removeRequestRowFromControlCard(int RequestId)
	{
		try
		{
			_unitOfWork.BeginTransaction();
			Request objRequestId = _unitOfWork.Repository<Request>().GetQueryAsNoTracking(Q => Q.Id == RequestId).SingleOrDefault();
			_unitOfWork.Repository<Request>().Delete(objRequestId);
			_unitOfWork.SaveChanges();
			List <RequestStatus> RequestStatusId = _unitOfWork.Repository<RequestStatus>().GetQueryAsNoTracking(Q => Q.RequestId == RequestId).ToList();
			_unitOfWork.Repository<RequestStatus>().DeleteRange(RequestStatusId.ToArray());
			_unitOfWork.SaveChanges();
			_unitOfWork.Commit();
			return new ResponseViewModel<InstrumentViewModel>
			{
