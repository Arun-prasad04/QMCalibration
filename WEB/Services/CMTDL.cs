...
namespace WEB.Services
{
    public class CMTDL
    {
        ...
        //GetRequestList
        public DataSet GetRequestList(int userid, int userroleid, int Startingrow, int Endingrow, string Search, string ReqType, string sscode, string instrumentname, string instrumentid, string status, string requestno, string requestdate, string range, string typeofrequest, string typeofequipment)
        {
        ...
        public DataSet GetInstruentList(int userid, int userroleid, int Startingrow, int Endingrow, string Search, string sscode, string instrumentname, string labid, string typeOfEquipment, string serialno, string range, string department, string calibrationdate, string duedate, string chkDue)
        {
            // string sSearch = Search.Equals(null) ?string.Empty:Search;
            var connectionString = _configuration.GetConnectionString("CMTDatabase");
            SqlCommand cmd = new SqlCommand("GetInstrumentList");
            ...
            cmd.Parameters.AddWithValue("@chkdue", chkDue);
		...
		*/
        ...
        public string CalculateDuedate(int calibFreq)
        {
            DateTime today = DateTime.Today;
            DateTime nextMonth = today.AddDays(1).AddMonths(calibFreq).AddDays(-1);
            DateTime endOfMonth = new DateTime(nextMonth.Year, nextMonth.Month, 1).AddMonths(1).AddDays(-1);
            return endOfMonth.ToString();
        }

        public string CalculateDuedate_old(int calibFreq)
        {
            string datefrq = null;
            try
            {
                DataSet ds = CalibrationDueDateCalCulation(calibFreq);

                if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    datefrq = Convert.ToString(ds.Tables[0].Rows[0]["DueDate"]);
                }

            }
            catch (Exception e)
            {
                ErrorViewModelTest.Log("CMTDL - CalculateDuedate Method");
                ErrorViewModelTest.Log("exception - " + e.Message);
            }
            return datefrq;
        }

        public DataSet CalibrationDueDateCalCulation(int calibFreq)
        {
            var connectionString = _configuration.GetConnectionString("CMTDatabase");
            SqlCommand cmd = new SqlCommand("CalibrationDueDateCalCulation");
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@calibFreq", calibFreq);
            SqlConnection sqlConn = new SqlConnection(connectionString);
            DataSet dsResults = new DataSet();
            SqlDataAdapter sqlAdapter = new SqlDataAdapter();
            cmd.Connection = sqlConn;
